 ...
 * training) unless an enterprise/zero-retention contract is in place (A3,
 * ported from the July line). Because a scan sends client source as prompt
 * context, these are classified `data_retaining` and BLOCKED under `block`
 * mode unless the operator declares an enterprise tier — a security product
 * must not silently ship client code to a training-retaining key (golden
 * rule #1 / #4).
 */
 ...
 * - Any other direct key (Anthropic, OpenAI, Google, xAI) cannot be confirmed
 *   from the key string → `unknown`.
 */
