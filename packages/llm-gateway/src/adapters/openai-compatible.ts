...
/**
 * Generic OpenAI **Chat Completions**-compatible adapter (the `openai` SDK with a
 * per-provider `baseURL` + Bearer key). One adapter covers every BYO-key provider
 * that speaks the OpenAI wire format: `openai` (direct GPT), `xai` (Grok),
 * `moonshot` (Kimi), `zhipu` (GLM), `deepseek`, and `google` via Gemini's
 * OpenAI-compatibility endpoint. Azure keeps its own adapter (tenancy client +
 * api-version). The logical model id is passed to the provider verbatim.
 */
