...
Overview Tab: Renders the 6-stage LayerProgress stepper and AppMapSummary. Live Agent Trace (A14): apps/web/src/components/layer-progress.tsx's LayerProgress renders a new AgentTrace panel under whichever layer is currently active — a small, scrollable list of that layer's recent progress messages (not just the single latest one), sourced from apps/web/src/lib/progress.ts's new layerNarrativeTrace helper over the SAME polled ProgressEvent array the stepper already fetches (no new transport). Most visible on Layer 3: the agentic investigation loop (E1) now streams its reasoning live via gateway.stream() (see docs/modules/confirmation.md's Streaming Investigation Narrative entry), so a multi-minute investigation shows a running trace instead of a single static phase line. Renders nothing when a layer has no more history than its one summary line.
...
Blue Team Tab: apps/web/src/app/scans/[scanId]/blue-team/page.tsx (B11). Six nested sub-tabs over `Report.blueTeam` (B10): Detection Rules (DetectionRulesPanel — generated Sigma/OTel/SIEM rules per confirmed finding, MITRE tags, log-signature narrative, per-rule and export-all download), ATT&CK Matrix (AttackHeatMap — a real tactics-as-columns/techniques-as-cells MITRE heat map, cell intensity reusing the severity color scale), Attack Paths (AttackPathsPanel — ordered kill-chain step cards with feasibility score), Purple Team (PurpleTeamPanel — detected/undetected scenario verification results with "why not" reasoning), Threat Model (ThreatModelPanel — B7's rendered markdown artifact), and Hardening (HardeningPanel — B9's advisory-only config/infra recommendations). Viewer-visible like the Report tab (read-only report data, no mutations) — EXCEPT the Detection Rules panel's new "Push to Splunk"/"Push all to Splunk" buttons (suggested enhancement, 2026-09-12 red/blue agentic-posture audit), which are genuine mutations (`POST /detection-rules/push{,-bundle}`) gated to Operator/Approver (`canPushDetectionRule`, lib/rbac.ts) and rendered only once `GET /detection-rules/push-targets` reports a configured target — invisible to a Viewer or an unconfigured client, never a dead/disabled button with no explanation.
