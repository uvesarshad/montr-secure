...
Air-Gapped Installation: Managed via scripts in deploy/airgap, this mode enables execution in disconnected networks. The build-bundle script creates Cosign-signed tar archives containing deterministic Semgrep rulesets and vulnerability advisory mirrors. The import-bundle script cryptographically verifies bundle signatures or SHA-256 checksums before deploying rule packages under its --dest-dir (default /opt/montr/airgap), grouped by artifact type. Semgrep rulesets installed under <dest-dir>/semgrep are consumed at scan time by packages/discovery/src/detectors/sast.ts once the discovery.rulesetsDir config key (env MONTR_DISCOVERY_RULESETS_DIR) is pointed at that path; SAST is a required detector and fails the scan rather than degrading silently if that path is missing or empty. OSV/GHSA advisory mirror artifacts are staged by import-bundle but have no runtime consumer yet.
