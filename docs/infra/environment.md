...
MONTR_FIX_AGENT_LOOP_MAX_TOOL_CALLS: Maximum sandboxed read_file tool round-trips per finding when the agent loop is enabled. Defaults to 0 (no tool use — the loop retries but cannot read other files), capped at 20. Tool round-trips are bounded separately and never consume a proposal attempt. Ignored when MONTR_FIX_AGENT_LOOP_ENABLED is false. Consumed by packages/config schema key fixGeneration.agentLoop.maxToolCalls.
