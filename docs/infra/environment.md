...
Reporting Variables (A18)
MONTR_REPORTING_EXECUTIVE_SUMMARY_ENABLED: Boolean toggle for Layer 5's optional AI-generated executive narrative (A18, 2026-09-12 red/blue agentic-posture audit) — `packages/report/src/executive-summary.ts`'s `generateExecutiveSummary`, called AFTER `buildReport` completes and attached at `Report.generatedExecutiveSummary`. Defaults to false — `packages/report` makes zero LLM calls and the report is template-assembled exactly as before, matching MONTR_FIX_AGENT_LOOP_ENABLED/MONTR_SEMANTIC_INDEX_ENABLED's off-by-default convention (a deliberate contrast with the same session's MONTR_CONFIRMATION_INVESTIGATION_ENABLED default-ON exception — two behavior/cost changes defaulting on in the same session was judged unnecessary risk). When true, one additional gateway call is made per scan at Layer 5, using the SAME shared gateway instance the rest of the scan uses (the existing MONTR_BUDGET_MAX_USD / MONTR_BUDGET_MAX_TOKENS / MONTR_BUDGET_ENFORCEMENT hard-halt above applies automatically, no separate wiring). Any gateway error or unusable reply degrades to `Report.generatedExecutiveSummary` simply staying absent — never blocks or fails the scan. Consumed by packages/config schema key reporting.executiveSummary.enabled, read by apps/worker/src/runners.ts's Layer 5 runner.
MONTR_REPORTING_EXECUTIVE_SUMMARY_MAX_TOKENS: Maximum output tokens for the one executive-summary generation call. Defaults to 1024, capped at 4096. Ignored when MONTR_REPORTING_EXECUTIVE_SUMMARY_ENABLED is false. Consumed by packages/config schema key reporting.executiveSummary.maxTokens.
