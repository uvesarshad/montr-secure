...
Per-Tenant Queue Isolation (A27, opt-in, OFF by default): packages/contracts/src/queue.ts's resolveQueueName(layer, clientId, tenantIsolation) derives the Redis queue name to use — QUEUE_NAMES[layer] unchanged when tenantIsolation is false (today's single-tenant on-prem default), or a per-client name (montr.layer0.<clientId>, sanitized to [A-Za-z0-9_-]) when true. The new config.queue (packages/config/src/schema.ts's QueueConfigSchema — perTenantIsolation default false, tenantIds default []; env MONTR_QUEUE_PER_TENANT_ISOLATION / MONTR_QUEUE_TENANT_IDS) is read by packages/orchestrator/src/bullmq-scheduler.ts's deriveTenantSchedulerOptions(config), which both apps/worker (apps/worker/src/index.ts's startWorker) and apps/api's produce-only scheduler (apps/api/src/production-deps.ts, via apps/api/src/enqueue-scheduler.ts's EnqueueOnlyScheduler) call against the SAME loaded config, so producer and consumer always agree on queue names. When enabled, BullMqJobScheduler.start() fans out one BullMQ Queue + one BullMQ Worker per (layer, tenantId) pair instead of one per layer — each Worker independently polls Redis, so a large backlog on one tenant's queue cannot delay a newly-queued job on another tenant's queue for the same layer (no shared head-of-line). This repo ships plain (non-Pro) BullMQ, which has no built-in group-based fair-scheduling primitive, so this is deliberately N-queues-per-tenant rather than a lighter-weight mechanism; QueueConfigSchema's doc comment recommends sharding tenants across multiple worker processes/pools rather than growing tenantIds unbounded on one process.
