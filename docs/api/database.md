...
- `DetectionRule`: one generated detection rule (Sigma YAML, an OTel query, or a SIEM query — `format` is a plain String column, not a Prisma enum, so B3 can add more formats without a migration, mirroring `RedTeamScenario.category`) for one `ConfirmedFinding`. `mitreTechniques` is a Json `string[]` of ATT&CK technique ids (B2 populates the mapping table that produces these). `provenance` reuses the existing `ProofType` enum ("static" | "live") to record whether the rule was derived from a static data-flow path or a live-DAST transcript, mirroring `ConfirmedFinding.proofType`.
