...
Detection-Rule Push Endpoints in apps/api/src/routes/detection-rules.ts (suggested enhancement, 2026-09-12 red/blue agentic-posture audit)
POST /api/v1/detection-rules/push-targets: Configures this client's real push destination for generated detection rules (Splunk HTTP Event Collector — the only implemented target; see docs/modules/reporting-vcs.md). Requires strict approver role and CSRF header (it stores a live, reversible outbound credential — the same sensitivity class as DAST target authorization). Accepts type (currently only "splunk_hec"), endpointUrl, hecToken, and optional index/sourcetype. Upserts one row per client, AES-256-GCM encrypting hecToken at rest, and records a detection_rule.push_target_configured audit event. Never returns the token in the response.
GET /api/v1/detection-rules/push-targets: Returns this client's configured push target as metadata only (type, endpointUrl, index, sourcetype, timestamps) — never the token. Requires operator or approver role.
DELETE /api/v1/detection-rules/push-targets: Removes this client's configured push target. Requires strict approver role and CSRF header. Records a detection_rule.push_target_deleted audit event.
POST /api/v1/detection-rules/push: Pushes one generated detection rule (sent inline in the request body — the console already holds it from the loaded Report; there is no server-side lookup by rule id, since the real StateStore.detectionRules table has no production writer yet) to the configured target. Requires operator or approver role and CSRF header. Returns 409 if no target is configured, propagates a 403 EGRESS_BLOCKED if the endpoint host is not on the security.allowedEgressHosts allowlist, and returns 502 DETECTION_RULE_PUSH_FAILED with the real reason on a non-2xx/transport failure from the target — never a silent success. Records detection_rule.pushed or detection_rule.push_failed.
POST /api/v1/detection-rules/push-bundle: Pushes every rule in an inline array (up to 200) to the configured target, sequentially. Requires operator or approver role and CSRF header. Never aborts on one rule's failure — returns a per-rule { ruleId, success, message } array so a partial failure is always visible, never swallowed.
