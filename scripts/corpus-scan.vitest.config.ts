...
/**
 * Vitest config for the golden-corpus REAL-MODE scan driver
 * (`scripts/corpus-scan.run.test.ts`, run via `node scripts/corpus-scan.mjs` /
 * `pnpm corpus:scan`). This is a SEPARATE config from the root `vitest.config.ts`
 * (not merged into it) so the driver — which shells out to real semgrep/gitleaks
 * and drives the full L0-L5 pipeline over every golden-corpus repo — is never
 * swept up by the ordinary `pnpm test` / `vitest run` run; it only runs when
 * explicitly invoked.
 *
 * The `resolve.alias` block mirrors the root `vitest.config.ts` (tests run
 * against package SOURCE, not built dist, same as everywhere else in this repo).
 * Keep the two in sync if a package is added/removed.
 */
export default defineConfig({
  ...
  test: {
    environment: "node",
    include: [src("./corpus-scan.run.test.ts")],
    exclude: ["**/node_modules/**", "**/dist/**"],
    // Real semgrep/gitleaks subprocesses + a multi-repo L0-L5 pipeline run are
    // slow (registry ruleset fetch, several repos) — well above vitest defaults.
